...
gge.matrix <- function(x, center=TRUE, scale=TRUE,
  ...
  if(!is.element(method, c('svd', 'nipals', 'bpca')))
  ...
  if(method=="svd"){
    ...
    x.svd <- NULL
    x.pca <- pca(x, method='bpca',nPcs=maxPCs-1)

    R2 <- x.pca@R2
    # estimate missing values to get complete data for calculating
    # 
    x[is.na(x)] <- completeObs(x.pca)[is.na(x)]
    x <- scale(x, center=center, scale=scale)
    fit <- completeObs(x.pca)
  ...
  ret <- list(x=x, x.orig=x.orig,
              ...
              pctMiss=pctMiss, maxPCs=maxPCs, fitted=fit)
  class(ret) <- "gge"
